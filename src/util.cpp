...
const char * const BITCOIN_CONF_FILENAME = "noodlyappendagecoin.conf";
const char * const BITCOIN_PID_FILENAME = "noodlyappendagecoind.pid";
...
#else
    const char* pszModule = "noodlyappendagecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Noodlyappendagecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Noodlyappendagecoin";
#else
    // Unix
    return pathRet / ".noodlyappendagecoin";
#endif
#endif
}
